      ...
      {stock.map((item, idx) =>
        item.type === "header" ? (
          ...
          <div key={idx} className="item-row">
            ...
            <div>
              <button className="btn btn-minus" onClick={() => updateStockQty(idx, -1)}>-</button>
              <span style={{ margin: "0 10px" }}>{item.qty}</span>
              <button className="btn btn-plus" onClick={() => updateStockQty(idx, 1)}>+</button>
            </div>
          </div>
        )
      )}
      <a href={createStockMessage()} target="_blank" rel="noopener noreferrer">
        <button className="send-btn">Send on WhatsApp</button>
      </a>
    </>
